...
template <class Scalar>
void
WellContributions<Scalar>::addMatrix([[maybe_unused]] MatrixType type,
                                     ...
                                     [[maybe_unused]] unsigned int val_size)
{
    ...
    OPM_THROW(std::logic_error,
              "Error cannot add StandardWell matrix on GPU because neither CUDA nor OpenCL were found by cmake");
#endif
...
template <class Scalar>
void
WellContributions<Scalar>::setBlockSize(unsigned int dim_, unsigned int dim_wells_)
{
    dim = dim_;
...
template <class Scalar>
void
WellContributions<Scalar>::setVectorSize(unsigned N_)
{
    N = N_;
}
...
template <class Scalar>
void
WellContributions<Scalar>::addNumBlocks(unsigned int numBlocks)
{
    if (allocated) {
...
template <class Scalar>
void
WellContributions<Scalar>::alloc()
{
    if (num_std_wells > 0) {
...
template <class Scalar>
void
WellContributions<Scalar>::addMultisegmentWellContribution(unsigned int dim_,
